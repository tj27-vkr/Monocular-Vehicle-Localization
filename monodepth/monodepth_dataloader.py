...
class MonodepthDataloader(object):
    ...
    def __init__(self, data_path, filenames_file, params, dataset, mode):
        ...
        if mode == 'train':
            ...
            left_image.set_shape( [None, None, 1])
            right_image.set_shape([None, None, 1])
            ...
            self.left_image_batch.set_shape( [2, None, None, 1])

            if self.params.do_stereo:
                self.right_image_batch = tf.stack([right_image_o,  tf.image.flip_left_right(right_image_o)],  0)
                self.right_image_batch.set_shape( [2, None, None, 1])
    ...
    def augment_image_pair(self, left_image, right_image):
        ...
        color_image = tf.stack([white * random_colors[i] for i in range(1)], axis=2)
